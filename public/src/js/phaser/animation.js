PWG.PhaserAnimation = function() {
	...
	function AnimationController(config, controller) {
		trace('AnimationController, config = ', config, '\tcontroller = ' + controller.name)
		this.config = config;
	...
	module.addAnimations = function(config, view) {
		...
		trace('====== controler = ', controller);
		module.controllers[controller.name] = controller;
	};
	
	module.play = function(view, animation, killOnComplete) {
		trace('PhaserAnimation/play, animation = ' + animation + ', view = ' + view + ', controllers = ', module.controllers);
